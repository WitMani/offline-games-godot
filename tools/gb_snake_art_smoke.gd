...
const MODEL_SHA := "124906afc511be63ba2abd61aff05dfa6e5bb51f076bfe1ade48b9168b7dbe06"
...
const REQUIRED_COPY := [
	"收盒", "重开", "实体方向键转向 · 长度达到 120", "按右上角重开",
	"GB SNAKE · FIELD LOG", "LEN 120", "TARGET CLEAR", "GAME OVER",
]
...
func _test_runtime_assets() -> void:
	...
	_expect(FileAccess.get_sha256("res://models/snake_gb_model.gd") == MODEL_SHA, "model_changed")
...
func _test_initial_signature() -> void:
	...
	_expect(game.state.get("food", []) == [11, 11], "initial_lure_cell")
	_expect(int(game.state.get("target_length", 0)) == 120, "target_length")
...
func _test_forage_feedback() -> void:
	...
	game.snake_gb_model.food = old_head + Vector2i.RIGHT
	...
	_expect(int(game.state.get("pending_growth", 0)) == 1, "forage_pending_growth")
...
func _test_completion_feedback() -> void:
	...
	_expect(int(game.state.get("score", 0)) == 120, "complete_score")
	_expect(str(game.state.get("terminal_reason", "")) == "target", "complete_reason")
	_expect(_last_sfx() == GAG_COMPLETE, "complete_gag_sound")
