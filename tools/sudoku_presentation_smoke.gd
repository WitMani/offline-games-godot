...
const REQUIRED_COPY := ["红笔修正", "轻轻擦去", "落笔正确", "九宫完成", "整册完成", "逻辑完成", "点击右上角“重开”继续挑战"]
...
func _test_routine_correct() -> void:
	...
	_expect(int(game.state["board"][0][2]) == int(solution[0][2]), "correct_value")
	_expect(int(game.state["moves"]) == 1, "correct_moves")
	_expect_event("logic_correct", 1, "落笔正确")
	_expect_presenter("logic_correct", 1, Vector2i(2, 0))
...
func _test_erase() -> void:
	...
	_expect(int(game.state["board"][0][0]) == 0, "erase_value")
	_expect(int(game.state["moves"]) == 1, "erase_moves")
	_expect_event("logic_erase", 1, "轻轻擦去")
	_expect_presenter("logic_erase", 1, Vector2i(0, 0))
...
func _test_error() -> void:
	...
	_expect(game.state["board"] == before, "error_board_unchanged")
	_expect(int(game.state["moves"]) == 0, "error_moves_unchanged")
	_expect(int(game.state["mistakes"]) == 1, "error_mistake_increment")
	_expect_event("logic_error", 2, "红笔修正")
	_expect_presenter("logic_error", 2, Vector2i(2, 0))
...
func _test_block_completion() -> void:
	...
	game.state["selected"] = [2, 0]
	game._sudoku_place(int(solution[0][2]))
	_expect(game._sudoku_block_complete(0), "block_state")
	_expect(str(game.state["status"]) == "playing", "block_not_terminal")
	_expect_event("logic_block_complete", 3, "九宫完成")
	_expect_presenter("logic_block_complete", 3, Vector2i(2, 0))
...
func _test_global_completion() -> void:
	...
	_expect_presenter("logic_complete", 4, Vector2i(0, 0))
