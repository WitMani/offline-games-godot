...
const OUTPUT := "res://docs/audit/amaze-v3/art/performance.json"
const SOLUTION := [
...
func _run() -> void:
	...
	_open_level_three()
	var busy := await _collect(300, true)
	_open_level_three()
	...
	var report := {
		...
		"presentation":"paint-workshop-gag-v2",
		...
		"busy_case":"continuous deterministic level-three solution loop; ordered wet-ribbon reveal, GAG paint-pod travel, routine/revisit/near/complete event grammar",
		"catalog_effect_cap":12,
		"runtime_asset_allocation":"preloaded texture/audio; no per-frame image or stream construction",
		"note":"Regression trace for llvmpipe in this environment; not a physical-device FPS claim.",
	}
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(OUTPUT.get_base_dir()))
	var file := FileAccess.open(ProjectSettings.globalize_path(OUTPUT), FileAccess.WRITE)
	file.store_string(JSON.stringify(report, "  ") + "\n")
	file.close()
	print("AMAZE_ART_PERFORMANCE=%s" % ProjectSettings.globalize_path(OUTPUT))
	quit()
